...
exports.index = function (req, res) {
  Workentry.find({ user: req.user._id }).populate('user',['name','_id','role']).exec(function (err, workentrys) {
...
// show All Work Entry
exports.allWorkEntry = function (req, res) {
  Workentry.find({}).populate('user',['name','_id','role']).exec(function (err, workentrys) {
    if (err) { return handleError(res, err); }
    return res.json(200, workentrys);
  });
};
